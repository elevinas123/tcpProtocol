...
int createSendingSocket() {
    ...
    int main() {
        ...
        if (choice == 0) {
            ...
            int messageSent = sendMessage(message_sockfd, "", sequenceNumber, 0, 1, 0, messagePort, true);
            sequenceNumber++;
            ...
            while (true) {
                ...
                // Receive the packet
                int data_size =
                    recvfrom(receive_sockfd, buffer, sizeof(buffer), 0, &saddr, &saddr_len);
                if (data_size < 0) {
                ...
                if (tcph->syn == 1 && tcph->ack == 1 && !handshakeCompleted) {
                    if (ackNumberReceived == sequenceNumber) {
                        ...
                        int ackSent = sendMessage(message_sockfd, "", sequenceNumber,
                                                  seqNumberReceived + 1, 0, 1, messagePort, true);
                ...
                if (handshakeCompleted && messagesSent < lines.size()) {
                    for (int i = 0; i < messageInfoVector.size(); i++) {
                        ...
                        if (sendMessage(message_sockfd, messageInfo.messageSent,
                                        messageInfo.seq_end, ackNumberReceived, 0, 0,
                                        messagePort, true) != 0) {
                            std::cout << "Failed To Send Message" << std::endl;
                        }
                        std::cout << "Message Sent, Payload: " << messageInfo.messageSent
                                  << std::endl;
                        messagesSent++;
                    }
                }
