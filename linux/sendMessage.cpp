...
int sendMessage(int sockfd,
    ...
    memcpy(buffer + sizeof(struct iphdr) + sizeof(struct tcphdr), message, message_len);
    ...
    // Recalculate IP checksum (optional, since it's 0 for now)
    iph->check = checksum((unsigned short *)buffer, iph->ihl * 4);

    // Step 6: Define destination address and send the packet
    struct sockaddr_in dest;
    dest.sin_family = AF_INET;
    dest.sin_port = tcph->dest;  // Destination port
    dest.sin_addr.s_addr = iph->daddr;

    // Send the packet
    if (sendto(sockfd, buffer, ntohs(iph->tot_len), 0, (struct sockaddr *)&dest, sizeof(dest)) <
        0) {
